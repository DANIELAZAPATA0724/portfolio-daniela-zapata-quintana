...
export class AuthRegisterComponent {
  ...
  navigateToApp() {
    //Ruta que vueve a landing al pulsar btn
    this.router.navigate(['']);
  }

  navigateToAuthLogin() {
    //Ruta que navega a login al pulsar btn
    this.router.navigate(['/login']);
  }

  navigateToBlog() {
    //Ruta que navega a home al pulsar btn
    this.router.navigate(['/blog']);
  }

  // Custom validator function to check if passwords match
  passwordMatchValidator(control: AbstractControl) {
    // Change parameter type to AbstractControl
  ...
  areAllFieldsFilled(): boolean {
    const formValues = this.formNewUser.value as {
  ...
  onSubmit() {
    if (
      this.formNewUser.valid &&
      !this.formNewUser.errors?.['mismatch'] &&
      this.formNewUser.get('terms')?.value
    ) {
